...
function! s:extendNamespaces(namespaces, namespaceList)
    let namespace = ''
    for curNamespace in a:namespaces
        let namespace .= curNamespace.'::'
    endfor
    call map(a:namespaceList, 'namespace.v:val')
endfunc
    ...
    while curPos !=[0,0]
        ...
        if curPos!=[0,0] && curPos[0]!=lastLine
            ...
            if match(szLine, '\Cusing\s\+namespace')<0
                ...
                " We get the namespace list from the line
                let namespaceList = s:GetNamespaceListFromLine(szLine)
                let curScopeItem = s:GetClassScopeAtCursor()
                call s:extendNamespaces(curScopeItem.namespaces, namespaceList)
                let result = namespaceList + result
                let nextStopLine = curPos[0]
            endif
        endif
    endwhile
