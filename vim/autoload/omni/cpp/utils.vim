...
function! omni#cpp#utils#GetResolvedTagItem(namespaces, typeInfo)
    ...
    if s:IsTypeInfoResolved(szTypeInfo)
        ...
        " let szTagQuery = substitute(szTagQuery, '^.*::', '', 'g')
        let tagList = omni#common#utils#TagListNoThrow('^'.szTagQuery.'$')
        if len(tagList)==0
            for namespace in a:namespaces
                let tempTagQuery = namespace.'::'.szTagQuery
                let tempTagQuery = substitute(tempTagQuery, '^::', '', 'g')
                let tagList = omni#common#utils#TagListNoThrow('^'.tempTagQuery.'$')
                if len(tagList)
                    break
                endif
            endfor
...
let s:matchGroup1 = 'shared_ptr\|auto_ptr\|weak_ptr'
let s:matchGroup2 = 'list\|map\|vector\|set'
...
function! omni#cpp#utils#ExtractTypeInfoFromTokens(tokens)
    ...
    for token in tokens
        ...
        elseif state==1
            if token.kind == 'cppWord'
                let szResult .= token.value
                if match(token.value, s:matchGroup2)>=0
                    let state = 3
                else
                    let state = 2
                endif
    ...
    if match(szResult, s:matchGroup1) >= 0
        let szResult = substitute(szResult, '^.*<\|>.*$', '', 'g')
    endif
    return szResult
endfunc
